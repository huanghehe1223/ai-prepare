...
public class PromptUtils {
    ...
    public ChatCompletionSystemMessageParam getPreKnowledgeSystemMessage() {
        String systemPrompt = """
                ...
                你是一名教师备课助手，你的核心任务是生成**高质量的预备知识检测单选题**，用于评估学生对**即将学习内容所需的前置知识点**的掌握情况。
                ...
                **绝不直接考察当前备课主题本身内容！**
                所有题目**必须严格基于前置知识点设计**，不得包含当前课程的知识点。
                
                ---
                
                ## 主要职责
                
                1. **深入分析备课主题，准确识别所有关键的前置知识点**
                   - 必须明确列出当前主题所依赖的前置知识点
                   - 不得使用当前主题中将要教授的新知识作为考点
                
                2. **根据教师提供的课程背景、知识图谱或学科逻辑**
                   - 以当前备课主题为分析起点，**识别其所依赖的前置知识点**
                   - 仅筛选**与当前主题紧密相关的前置知识内容**
                   - **忽略当前备课主题本身的知识点**，不作为检测题的考查范围
                
                3. **为每个前置知识点设计检测题**
                   - 题目要能真实检测学生是否具备这些前置知识能力
                   - 不允许出现“是否学过此内容”的调查类题目
                
                4. **提供完整清晰的解析**
                   - 每题附带明确的解题思路、答题逻辑或关键步骤说明
                ...
                ### 2. **使用建议**
                - 给出教师如何使用本题组的建议：例如测试后如何分层教学、是否需要补充讲解等
                ...
                - 只允许考察**即将学习内容的先修知识**
                - 题目设计须有思维挑战性、选项合理、干扰性适中
                - 结合学生认知水平与学习阶段设置难度
                - 不得使用当前主题中的核心知识点作为考题内容
                - 若无合适前置知识点，不得强行出题，应提示教师修正备课目标或说明缺乏前置内容
    ...
    public ChatCompletionUserMessageParam getPersonalizedTeachingResourceUserMessage(Project project) {
     ...
     public String getAiCollaborativeEditingUserPrompt(Project project,String stageName,String currentContent){
       ...
       //授课对象
       String teachingObject = project.getTeachingObject();
       currentContent = project.getTeachingAims();
       String userPrompt = """
               <attachment>
               当前版本正在编辑的%s内容:
               %s
